 ...
 */
public class old_EntryComposite extends Composite implements IUpdateableListener {
	 ...
	 *            internally by ICE.
	 */
	public old_EntryComposite(Composite parent, int style, Entry refEntry) {
		...
		addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				entry.unregister(old_EntryComposite.this);
			}
		});
	...
	public void update(IUpdateable component) {
		...
		if (component == entry) {
			PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
				@Override
				public void run() {
					if (!old_EntryComposite.this.isDisposed()) {
						...
						// value is different
						if (!old_EntryComposite.this.entry.getValue().equals(currentSelection)) {
							toggleSaveDecoration();
						}

						// Update the reference to the Entry's value
						currentSelection = old_EntryComposite.this.entry.getValue();
		...
		if (decoration == null) {
			...
			editor.addPropertyListener(new IPropertyListener() {
				...
				public void propertyChanged(Object source, int propId) {
					...
					// value has changed
					if (editor != null) {
						if (editor.isDirty() && !old_EntryComposite.this.entry.getValue().equals(currentSelection)) {
							// Show the decoration
							old_EntryComposite.this.decoration.show();
						} else if (!editor.isDirty()) {
							// Hide the decoration
							old_EntryComposite.this.decoration.hide();
						}
					}
