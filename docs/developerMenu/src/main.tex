...
and exposing a new extension. Let's see how to do this in detail:
...
org.eclipse.ice.developer.code extension point that let's users define various
...
select New $>$ code in the context menu. This will present you with the view in
Figure \ref{fig:config2} where you can select the code category, code display
name, the repository URL, and the branch you would like to work with. 
...
name of this action and the AbstractHandler subclass that performs the action. ICE provides a
default GitCloneHandler that you may select. For this tutorial, select that and
click save. You should now have a view similar to the bottom of Figure
\ref{fig:config2}.
...
your new developer plugin (see Figure \ref{fig:launch}) by enabling it. The
